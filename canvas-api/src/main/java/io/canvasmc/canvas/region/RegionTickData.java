...
public interface RegionTickData {
     ...
     * {@link IRegionizedData#createValue(RegionTickData)}. Once created, the value will remain attached to this region
     * until killed or removed from splits or merges</p>
     *
     ...
     *
     * @param regionizedData
     *     The regionized data definition to retrieve or create.
     * @param <T>
     *     The type of the data value stored.
     *
     ...
     * are created, as well as how they should be handled when regions merge or split. The associated data instance can
     * be retrieved through {@link RegionTickData#getOrCreateFromIRegionizedData(IRegionizedData)}.</p>
     *
     * @param <T>
     *     The type of object that this regionized data stores.
     *
    ...
    interface IRegionizedData<T> {
        /**
         * Creates a new instance of this data type, called to initially create the instance that is added to the region
         * tick data
         *
         * @param tickData
         *     The region tick data requesting a new instance.
         *
         ...
         * is executed under critical locks, so operations must be completed in a quick and timely manner</p>
         *
        ...
        interface IRegionizedCallback<T> {
             ...
             * computing the absolute deadline in region {@code into} that has the same relative deadline as
             * {@code from} as {@code deadlineTo = relFrom + currentTickTo}. By substituting {@code relFrom} as
             * {@code deadlineFrom - currentTickFrom}, we finally have that
             * {@code deadlineTo = deadlineFrom + (currentTickTo - currentTickFrom)} and that we can use an offset
             * {@code fromTickOffset = currentTickTo - currentTickFrom} to calculate {@code deadlineTo} as
             * {@code deadlineTo = deadlineFrom + fromTickOffset}.
             * </p>
             * <p>
             * <b>Critical Notes:</b>
             * <li>
             ...
             * @param into
             *     The data to merge into.
             * @param fromTickOffset
             *     The addend to absolute tick deadlines stored in the {@code from} region to adjust to the into
             *     region.
             */
             ...
             * <p>
             * The chunk coordinate to region section coordinate bit shift amount is provided in
             * {@code chunkToRegionShift}. To convert from chunk coordinates to region coordinates and keys, see the
             * code below:
             * <pre>
             *         {@code
             *         int chunkX = ...;
             ...
             * the new regions formed from the split will start at the same tick number, and so no adjustment is
             * required.
             * </p>
             *
             * @param from
             *     The data to split from.
             * @param chunkToRegionShift
             *     The signed right-shift value used to convert chunk coordinates into region section coordinates.
             * @param regionToData
             *     Lookup hash table from region section key to .
             * @param dataSet
             *     The data set to split into.
             */
            void split(
