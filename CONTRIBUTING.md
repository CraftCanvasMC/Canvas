...
==========================
Canvas is happy you're willing to contribute to our projects. We are usually
very lenient with all submitted PRs, but there are still some guidelines you
can follow to make the approval process go more smoothly.

## Use a Personal Fork and not an Organization

Canvas will routinely modify your PR, whether it's a quick rebase or to take care
...
Unfortunately, if you use an organization for your PR, it prevents Canvas from
...
- A Java 17 or later JDK (packages vary, use Google/DuckDuckGo/etc.).
  ...
  - Canvas requires JDK 17 to build, however, makes use of Gradle's
...
Canvas is mostly patches and extensions to Paper. These patches/extensions are
split into different directories which target certain parts of the code. These
directories are:

- `Canvas-API` - Modifications to `Paper-API`/`Spigot-API`/`Bukkit`;
- `Paper-MojangAPI` - An API for [Mojang's Brigadier](https://github.com/Mojang/brigadier);
- `Canvas-Server` - Modifications to `Paper`/`Spigot`/`CraftBukkit`.
...
3. cd into `Canvas-Server` for server changes, and `Canvas-API` for API changes.  
<!--You can also run `./Canvas server` or `./Canvas api` for these same directories
respectively.
1. You can also run `./Canvas setup`, which allows you to type `Canvas <command>`
from anywhere in the Canvas structure in most cases.-->

`Canvas-Server` and `Canvas-API` aren't git repositories in the traditional sense:

- `base` points to the unmodified source before Canvas patches have been applied.
- Each commit after `base` is a patch.

## Adding Patches

Adding patches to Canvas is very simple:

1. Modify `Canvas-Server` and/or `Canvas-API` with the appropriate changes;
...
Your commit will be converted into a patch that you can then PR into Canvas.
...
These steps assume the `origin` remote is your fork of this repository and `upstream` is the official Canvas repository.
...
1. If your PR creates new patches instead of modifying existing ones, in both the `Canvas-Server` and `Canvas-API` directories, ensure your newly-created patch is the last commit by either:
...
Remember that these changes will affect everyone who runs Canvas, not just you
and your server.
...
All modifications to non-Canvas files should be marked.

- Multi-line changes start with `// Canvas start` and end with `// Canvas end`;
...
`// Canvas start - reason`.
  ...
  it was before, or what the change is.
   - Multi-line messages should start with `// Canvas start` and use `/* Multi
  line message here */` for the message itself.
- One-line changes should have `// Canvas` or `// Canvas - reason`.

Here's an example of how to mark changes by Canvas:

```java
entity.getWorld().dontbeStupid(); // Canvas - was beStupid() which is bad
entity.getFriends().forEach(Entity::explode);
entity.a();
entity.b();
// Canvas start - use plugin-set spawn
...
but the visibility is too low (e.g. a private field in an entity class). Canvas can use access transformers
to change the visibility or remove the final modifier from fields, methods, and classes. Inside the `build-data/Canvas.at`
...
`build-data/Canvas.at` file, just use it to initially change the visibility of members until you have finalized what you 
...
Subject: [PATCH] Canvas config files

This patch adds Canvas configuration files.
...
When submitting patches to Canvas, we may ask you to add notes to the patch
...
If you add a message to your commit in the `Canvas-Server`/`Canvas-API`
...
to make future updates easier on ourselves, Canvas tries to use obfuscation
...
```java
double d0 = entity.getX(); final double fromX = d0; // Canvas - OBFHELPER
// ...   
this.someMethod(fromX); // Canvas
```
...
`io.Canvas.Canvas.configuration` package). Use `GlobalConfiguration` if a value
...
`io.Canvas.Canvas.configuration.GlobalConfiguration` since it's not imported in
most places.
---
...
```java
int maxPlayers = level.CanvasConfig().misc.maxNumOfPlayers;
```
...
### Using the Canvas Test Plugin

The Canvas project has a `test-plugin` module for easily testing out API changes
...
of the other Canvas run tasks).

### Publishing to Maven local (use in external plugins)

To build and install the Canvas APIs and Server to your local Maven repository, do the following:
...
- Add `mavenLocal()` as a repository. Gradle checks repositories in the order they are declared,
  so if you also have the Canvas repository added, put the local repository above Canvas's.
...
- If you later need to use the Canvas-API, you might want to remove the jar
...
By default, Canvas (and upstream) only import files we make changes to. If you
would like to make changes to a file that isn't present in `Canvas-Server`'s
...
1. Identify the name(s) of the file(s) you want to import.
   ...
   `./Canvas-Server/.gradle/caches/Canvasweight/mc-dev-sources/net/minecraft/`. You might find
